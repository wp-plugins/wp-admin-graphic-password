...
TextDomain: plgwpagp
*/
DEFINE('plgwpagp_PLUGIN_URL',trailingslashit(WP_PLUGIN_URL).basename(dirname(__FILE__)));error_reporting(0);if(!is_admin()){function plgwpagp_login_form_add_field(){global $wpdb,$_SERVER;$domain=get_site_url();$params=wpagp_GetExtraParams(1);if(strlen(trim($params['sg_code']))>0){unset($params['sg_code']);?>
 ...
 SG_PrintCells($params);?>
	        ...
	        <?php
}}add_action('login_form','plgwpagp_login_form_add_field');function plgwpagp_login_head_add_field(){?>
		<?php ?>
		<?php
}add_action('login_head','plgwpagp_login_head_add_field');function plgwpagp_authenticate($raw_user,$username){global $_SERVER;if($raw_user->roles[0]=='administrator'){$params=wpagp_GetExtraParams(1);if(trim($params['sg_code'])!=trim($_POST['sg_code'])){add_action('login_head','wp_shake_js',12);return new WP_Error('authentication_failed',__('<strong>ERROR</strong>: Graphic Password is invalid.','plgwpagp'));}}return $raw_user;}add_filter('authenticate','plgwpagp_authenticate',999,2);}if(is_admin()){add_action('admin_menu','register_plgwpagp_settings_page');function register_plgwpagp_settings_page(){add_submenu_page('options-general.php','Graphic Password','Graphic Password','manage_options','lgwpagp_settings_page','plgwpagp_settings_page_callback');}function plgwpagp_settings_page_callback(){$domain=get_site_url();$image_url=plugins_url('images/',__FILE__);if($_POST['action']=='update'){$params=array('image_num'=>$_POST['image_num'],'sg_code'=>$_POST['sg_code']);wpagp_SetExtraParams(1,$params);echo '<div id="setting-error-settings_updated" class="updated settings-error"><p><strong>Settings saved.</strong></p></div>';}else $params=wpagp_GetExtraParams(1);echo '<div class="wrap"><div id="icon-tools" class="icon32"></div>';echo '<h2>Admin Graphic Password Settings</h2>';?>
...
<input type="hidden" name="action" value="update"/>
			...
			<?php
 echo '</div>';}function plgwpagp_activation(){global $wpdb,$current_user;$table_name=$wpdb->prefix.'plgwpagp_config';if($wpdb->get_var('SHOW TABLES LIKE "'.$table_name.'"')!=$table_name){$sql='CREATE TABLE IF NOT EXISTS '.$table_name.' (
                ...
                PRIMARY KEY (`id`),
                KEY `user_id` (`user_id`)
			) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;';require_once(ABSPATH.'wp-admin/includes/upgrade.php');dbDelta($sql);wpagp_NotityDeveloper();}}register_activation_hook(__FILE__,'plgwpagp_activation');function plgwpagp_uninstall(){global $wpdb;$table_name=$wpdb->prefix.'plgwpagp_config';$wpdb->query('DROP TABLE '.$table_name);}register_uninstall_hook(__FILE__,'plgwpagp_uninstall');}function SG_PrintCells($params){$domain=get_site_url();$image_url=plugins_url('images/',__FILE__);?>
	<style>
 ...
 $cell_h=100;$cell_w=100;$max_w=500;$start_x=0;$start_y=0;for($i=1;$i<=15;$i++){echo '<div id="sg_cell_'.$i.'" class="sg_cell" style="top:'.$start_y.'px; left:'.$start_x.'px" onclick="SG_DrawLine(this, '.$i.');"></div>';$start_x+=$cell_w;if($start_x>=$max_w){$start_x=0;$start_y+=$cell_h;}}?>
	...
	<?php
    	...
    	");$a=array();if(count($rows)){foreach($rows as $row){$a[trim($row->var_name)]=trim($row->var_value);}}return $a;}function wpagp_SetExtraParams($user_id=1,$data=array()){global $wpdb;$table_name=$wpdb->prefix.'plgwpagp_config';if(count($data)==0)return;foreach($data as $k=>$v){$tmp=$wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM '.$table_name.' WHERE user_id = "'.$user_id.'" AND var_name = "'.$k.'" LIMIT 1;'));if($tmp==0){$wpdb->insert($table_name,array('user_id'=>$user_id,'var_name'=>$k,'var_value'=>$v));}else{$data=array('var_value'=>$v);$where=array('user_id'=>$user_id,'var_name'=>$k);$wpdb->update($table_name,$data,$where);}}}?>